...
static void dtls_peer_address_get(struct tls_session_context *session_ctx,
				  ...
				  socklen_t *addrlen)
{
	socklen_t len = MIN(session_ctx->dtls_peer_addrlen, *addrlen);
					     ...
					     socklen_t addrlen)
{
	...
	NET_INFO("Need to swap session for [%s]:%d (current [%s]:%d)",
		 addr->sa_family == AF_INET ?
		 net_sprint_ipv4_addr(&net_sin(addr)->sin_addr) :
		 net_sprint_ipv6_addr(&net_sin6(addr)->sin6_addr),
		 addr->sa_family == AF_INET ?
		 ntohs(net_sin(addr)->sin_port) :
		 ntohs(net_sin6(addr)->sin6_port),
		 tls_ctx->active_session->dtls_peer_addr.sa_family == AF_INET ?
		 net_sprint_ipv4_addr(&net_sin(&tls_ctx->active_session->dtls_peer_addr)->sin_addr) :
		 net_sprint_ipv6_addr(&net_sin6(&tls_ctx->active_session->dtls_peer_addr)->sin6_addr),
		 tls_ctx->active_session->dtls_peer_addr.sa_family == AF_INET ?
		 ntohs(net_sin(&tls_ctx->active_session->dtls_peer_addr)->sin_port) :
		 ntohs(net_sin6(&tls_ctx->active_session->dtls_peer_addr)->sin6_port));

	SYS_SLIST_FOR_EACH_CONTAINER(&tls_ctx->sessions, session_ctx, node) {
		if (dtls_is_peer_addr_valid(session_ctx, addr, addrlen)) {
			NET_INFO("Found active session");
					  ...
					  socklen_t addrlen)
{
	...
	/* TODO Limit the maximum number of sessions/socket? */
...
static int dtls_server_switch_active_session_by_cid(struct tls_context *tls_ctx)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&tls_ctx->sessions, session_ctx, node) {
		...
		if (ret == 0) {
			NET_INFO("Got CID match for [%s]:%d (was [%s]:%d)",
				 addr.sa_family == AF_INET ?
				 net_sprint_ipv4_addr(&net_sin(&addr)->sin_addr) :
				 net_sprint_ipv6_addr(&net_sin6(&addr)->sin6_addr),
				 addr.sa_family == AF_INET ?
				 ntohs(net_sin(&addr)->sin_port) :
				 ntohs(net_sin6(&addr)->sin6_port),
				 session_ctx->dtls_peer_addr.sa_family == AF_INET ?
				 net_sprint_ipv4_addr(&net_sin(&session_ctx->dtls_peer_addr)->sin_addr) :
				 net_sprint_ipv6_addr(&net_sin6(&session_ctx->dtls_peer_addr)->sin6_addr),
				 session_ctx->dtls_peer_addr.sa_family == AF_INET ?
				 ntohs(net_sin(&session_ctx->dtls_peer_addr)->sin_port) :
				 ntohs(net_sin6(&session_ctx->dtls_peer_addr)->sin6_port));
...
static int dtls_server_switch_session_on_rx(struct tls_context *tls_ctx)
{
	...
	NET_INFO("Session not found, allocating new");
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
	/* TODO Loop and set for each session? Why doesn't mbed TLS store the
	 * hostname within config...
	 */
					...
					void *optval, socklen_t *optlen)
{
	...
	/* TODO This will be problematic for the multi-session case... */
						     ...
						     socklen_t *optlen)
{
	...
	/* TODO This will be problematic for the multi-session case... */
	ret = mbedtls_ssl_get_peer_cid(&context->active_session->ssl, &enabled, optval, &optlen_local);
	if (enabled) {
		*optlen = optlen_local;
	} else {
		*optlen = 0;
...
int ztls_close_ctx(struct tls_context *ctx, int sock)
{
	...
	/* TODO Loop over all active sessions? */
	...
	if (dest_addr != NULL) {
		...
		if (ret < 0) {
			NET_ERR("No session found for [%s]:%d",
				...
				dest_addr->sa_family == AF_INET ?
				ntohs(net_sin(dest_addr)->sin_port) :
				ntohs(net_sin6(dest_addr)->sin6_port));
			errno = ENOTCONN;
			return -1;
		}
	}
...
again:
	...
	if (ret < 0) {
		 ...
		 * be reset in other error cases
		 */
		/* TODO This needs some further investigation  */
		if (is_server) {
		...
		if (ret == MBEDTLS_ERR_SSL_TIMEOUT) {
			...
			/* TODO For the server, need to figure out a better way to timeout. */
			return -ENOTCONN;
		}
